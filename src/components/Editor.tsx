...
export default function Editor() {
  ...
  const convertWebmToMp4 = async (webmBlob: Blob): Promise<Blob> => {
    ...
    try {
      ...
      ffmpeg.on('log', ({ message }) => {
        ...
        if (message.includes('Processing') || message.includes('frame=')) {
          setFfmpegStatus('MP4 파일 생성 중...');
        }
      });
      ...
      // Fast copy transmux to MP4
      await ffmpeg.exec(['-i', 'input.webm', '-c', 'copy', 'output.mp4']);
      
      setFfmpegStatus('완료된 MP4 비디오 파일 읽는 중...');
  ...
  const saveVideoFile = async (blob: Blob) => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        setFfmpegStatus('스마트폰 갤러리에 저장하는 중...');
        ...
        reader.onloadend = async () => {
          try {
            ...
            await Share.share({
              title: '자막 비디오 저장',
              text: '폰 갤러리에 저장하거나 친구에게 공유하세요.',
              url: savedFile.uri,
              dialogTitle: '파일 저장 및 공유',
            });
          } catch (e: any) {
  ...
  const startRecording = () => {
    ...
    recorder.onstop = async () => {
      ...
      // Save natively or download via browser
      await saveVideoFile(mp4Blob);
    };
    ...
    <div className="min-h-screen flex flex-col md:flex-row gap-6 p-6 max-w-7xl mx-auto items-stretch">
      ...
      <div className="flex-1 min-h-[400px] bg-[#111] rounded-2xl border border-white/5 overflow-hidden flex flex-col order-1 md:order-2 self-start md:sticky md:top-6">
        ...
        <div className="flex-1 relative flex items-center justify-center bg-black group">
          ...
          <AnimatePresence>
            {(isRecording || isFfmpegProcessing) && (
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                ...
                {isFfmpegProcessing ? (
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <span className="text-lg font-bold text-white tracking-tight">MP4 자막 인코딩 중</span>
                      <span className="text-xs text-white/50 font-mono animate-pulse">{ffmpegStatus || 'FFmpeg 작업 진행 중...'}</span>
                    </div>
                  </div>
                ) : (
                  <>
